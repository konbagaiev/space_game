...
import {
  clamp01, loadAudioSettings, saveAudioSettings, effectiveGain, MUSIC_TRIM,
...
test('effectiveGain multiplies master × channel and honors the on-toggle', () => {
  ...
  assert.equal(effectiveGain(s, 'music'), 0.1);          // 0.5 × 0.4 × MUSIC_TRIM(0.5)
  assert.equal(effectiveGain(s, 'sfx'), 0.4);            // 0.5 × 0.8 (no trim on SFX)
...
test('effectiveGain applies MUSIC_TRIM only to the music bus (music is baked ~2× quieter)', () => {
  const s = { master: 1, music: 1, sfx: 1, musicOn: true, sfxOn: true };
  assert.equal(effectiveGain(s, 'music'), MUSIC_TRIM);   // music at full slider = the trim (half)
  assert.equal(effectiveGain(s, 'sfx'), 1);              // SFX at full slider = full (untrimmed)
  assert.equal(MUSIC_TRIM, 0.5);                          // guard the chosen half factor
});
