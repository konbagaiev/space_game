...
test('capLifted is FALSE whenever roam is false, for every inZone value (replay invariant)', () => {
  assert.equal(capLifted({ roam: false, inZone: false }), false);
  assert.equal(capLifted({ roam: false, inZone: true }), false);
  // and only lifted in roam AND outside a zone
  assert.equal(capLifted({ roam: true, inZone: false }), true);
  assert.equal(capLifted({ roam: true, inZone: true }), false);
});
