...
2. **Descriptor key renamed `asteroids` → `speedField`** (`{ color, layers: [{count,size,radius,depth,depthVar,opacity}] }`).
   **The now-dead `asteroids: {…}` block STAYS in each map descriptor for exactly one release** as a
   ...
   propagates the removal. This condition is repeated in DECISIONS §95 so it is discoverable.
...
```js
  // Player-locked wrapping speed field (was: an origin-anchored asteroid ring — DECISIONS §95).
...
field; only this up-close field uses the model — DECISIONS §71 + §95").
...
> the ship); intro-replay guard green. DECISIONS §95, docs/plans/2026-08-09-1410-player-locked-speed-field.md.

**`docs/DECISIONS.md`** — new **§95** (next free number; §94 is the just-committed cruise assist), placed
...
distant BACKDROP half by §95 (the backdrop is now a player-locked wrapping Points speed field); the mission
