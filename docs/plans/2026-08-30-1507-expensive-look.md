## 0. WHAT SHIPPED (2026-08-31) — read this before the plan below

**The plan body under the horizontal rule is the ORIGINAL BRIEF and is now largely HISTORY.** It designed a
...
- **The frame** is the historical two-pass one, drawn **straight to the canvas** in `main.js animate()`
  (`renderer.info.reset()` → `clear()` → `render(skyScene, camera)` → `clearDepth()` →
  `render(scene, camera)`), duplicated verbatim in the `?bench` `fullFrame`. Native canvas MSAA, **no tone
  mapping**, no offscreen buffer, no full-screen pass. `client/src/postfx.js` and `client/src/glow-layer.js`
  do not exist.
- **Glow is real point lights** — `client/src/engine-lights.js`: a FIXED pool on engine nozzles, rockets in
  flight and explosion flashes, built once at load (three bakes the count into every lit material's shader).
  Tier knob `post: { lights: N }` — High 16 / Balance 4 / Performance `null`, measured on a Redmi 15C.
  `?lights=N` overrides it; the `?tune` "Engine lights" folder is the live tuning rig.
- **No HDR gains anywhere.** `fxGain` / `exhaustGain` / `postGain` / `fxColor` / `hdrColor` / the plume's
  `uGain` are deleted. FX colours are the authored ones on every tier.
- **The parallax backdrop layer stays** (Step 5 below is still accurate), with its own `?tune` folder.
- **The hull emissive floor stays wired and ships at 0** (Step 6a's mechanism is accurate; its 0.25 value and
  its bloom-threshold reasoning are not).
- **The ~30% larger speed-field dust stays** (Step 2), minus the `linearLuma601` helper, which existed only
  to police a bloom threshold that no longer exists.
- `43-expensive-look` measures what is left: the layer's differential contribution and the backdrop-vs-hull
  ratio, re-pinned on the shipped frame (D13 floor 1.11×, `hullLit >= 120`).

### Status

Implemented, tested and documented. Not deployed. The A/B perf bench has NOT been run (D16, opt-in).
