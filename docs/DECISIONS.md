...
## 142. A new `weightClass` axis alongside `class`/`role` — and the class row OWNS its blast numbers

**Context.** The explosion-light work needed to know how big a death was and had nothing to ask. A ship
carried two class-ish fields and neither answered it: `stats.class` is the SOUND class (`fighter`/`capital`/
`player`, the `sfxFor('ship', class, …)` key, which also picks scene music), and `stats.role` is BEHAVIOUR
with size smuggled in (`rocketeer` says what a hull carries, `medium` says how big it is). So `blastClass`
guessed from `sizeScale` thresholds — `medAt 1.4` / `bigAt 2.2`, numbers tuned against today's catalog hull
scales that would silently misclassify the first ship authored at 1.35.

**Decision.** A third axis, `stats.weightClass`, on every ship row, described by a `SHIP_CLASSES` table
(`client/src/sim-core/ship-classes.js`). Exactly ONE consumer is rewired: the blast flash.

- **Why not rename `class` into this.** `class` is the SFX key, it is in the sound map (`db.js`), and it is
  in recorded traces and on the netsim wire. Renaming it would be a data migration for a naming preference.
- **Why not migrate `role` yet.** "How does it behave" and "how heavy is it" are genuinely different
  questions; `role` still routes AI, the boss explosion and the louder boom. One consumer at a time — the
  ladder (migrating `role`, reward/XP by class, equipment restrictions) is on the roadmap, not in this change.
- **Why both bosses are `heavy`, and `ultraHeavy`/`station` are declared but unused.** The ladder is worth
  documenting in one place, and an unused row costs one line. A row with *invented* blast numbers would be a
  lie, so an unused class carries **no** blast block at all and the resolver falls back for it. `station` is
  declared for the record only: the Space Factory is a `world.js` set-piece, not a ships row.
- **Why the class row owns the literal `power`/`reach`/`durMul` rather than naming a shared tier.** A weight
  class you cannot re-tune on its own is not an axis, it is an alias. Because the numbers live in the row,
  the `?tune` "Blast flashes" folder is now GENERATED by iterating `SHIP_CLASSES` — adding a class makes its
  three sliders appear with no code edit. That is the property the whole feature is for, at the one place the
  maintainer would actually feel it.
- **The fallback chain stays forever:** resolvable `weightClass` with a blast block → else `isBoss` (a real
  boss must never be demoted by a modest scale) → else the `sizeScale` thresholds. It is not transitional
  scaffolding: recorded traces and payloads from an older server carry no class, and the `?tune` test range
  nulls the field on purpose so it keeps exercising exactly that path while faking hull sizes.

**Cost.** `engine-lights.js` imports `three`, so nothing in it can be unit-tested; the tiers + classifier
moved to a pure `client/src/blast.js` (re-exported from `engine-lights.js`, so no consumer's import changed).
That bought the guard this axis needs to not rot: `client/src/blast.test.js` reads the real
`server/src/catalog_seed.js` and pins every ship's peak/reach/duration to a golden table, so the change is
provably invisible — all 10 ships classify exactly as the thresholds did.

## 143. A finished mission closes on ARRIVAL — `canDock` is relaxed, but only under `finishing`

**The bug, found by playing (2026-08-31).** Clear a sector, press "Finish and Return", then fire once. Firing
cancels the autopilot (§39). From that point the mission could not be closed at all:

1. the button refused every press after the first — `finishMission` bailed on `lr.finishing` already true;
2. flying home by hand did nothing — `checkArrival` required `canDock(...)`, i.e. an ENGAGED station autopilot.

The only way out was clicking the station with the mouse to re-engage the autopilot, which nothing on screen
communicates. The reward was already banked and the advance already committed, so the player was not losing
anything except the ability to leave — which is worse, because it looks like the game has hung.

**Considered and rejected: make firing stop cancelling the autopilot.** That is §39's whole point — any
control input hands control straight back — and a cruise you cannot break out of by shooting is a worse
problem than the one being fixed. The maintainer ruled it out explicitly. Two narrower fixes instead:

- **The button re-engages.** When `lr.finishing` is already true but the station autopilot is no longer
  engaged, `finishMission` re-engages it and returns true. It still refuses when the sector is not cleared or
  the mission is already won, and the **one-shot settlement is not repeated**: `collectAll` and the
  `finishing` event belong to the moment the player decided, and a second `finishing` would ask the host to
  commit the same campaign advance twice.
- **Arrival counts once `finishing` is true.** `checkArrival` drops the `canDock` requirement down to plain
  distance — but ONLY under `finishing`.

**Why the relaxation is safe exactly there, and nowhere else.** `canDock` exists to enforce two things
(`autopilot-config.js`): proximity alone must never end a mission, and a chest-aimed autopilot must be
*structurally* incapable of winning one. Both are about a mission the player has **not** decided to end. Once
`finishing` is true he has pressed the button, the sector is cleared, the credits are doubled, the XP bonus
has landed, the salvage is swept and the advance is committed — every decision is made and every reward is
banked. All that remains is the trip, so "did you arrive" is the only question left, and `canDock` is
answering a question that is no longer being asked. The gate is on `lr.finishing` alone, so every other path
still goes through `canDock` unchanged; the negative tests in `level-runner.test.js` are the ones that matter
here, and they were mutation-checked against a version that relaxes the guard unconditionally.

**No "settled" check on the new path**, deliberately. `checkStationArrival` (step-player.js) requires
`vel <= 0.6` because it PARKS the ship and holds it still under a prompt. This closes the mission and raises
the victory overlay, where nothing needs the ship stationary — and requiring a near-stop would be a second
trap of the same family: a player hand-flying home at full speed crosses the 45-unit radius in well under a
second, and no HUD element would tell him to brake.

**Superseded in passing: the blue homing arrow is GONE (2026-08-31).** §39 and the entries around it
describe a world-space arrow that pointed the way home; `updateReturnArrow` has been deleted outright. It had
no distance gate, so in roam it switched on from the first frame after takeoff and pointed at a station
fifteen units away that the player could plainly see — noise that also devalued it for the one case it was
