...
- **`makeReplaySession()` (`replay.js`)** — the playback/cutscene lifecycle state is one object (owned
  fields `play`/`trace`/`armed`/`index`/`done`/`cut`/`cutDone`/`cutReturning`; an `active` getter = the
  `animate()` gate `!!rs.play`; a unit-tested `teardown()` that clears them all together). `main.js` holds
