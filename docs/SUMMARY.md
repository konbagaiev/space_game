...
- Background in 3 layers: stars (varying brightness, a static backdrop) → the speed field (the parallax
  layer) → planet + 2 moons (light parallax). **Stars are two point layers (`makeStars`):** the dim majority
...
- **The parallax layer is a PLAYER-LOCKED WRAPPING SPEED FIELD** (`THREE.Points`, DECISIONS §95) — its only
...
- **The whole scene is data-driven:** it's described by a JSON **map descriptor** in the DB (`maps`
  ...
  (planet/moons/stars/speed field/sky-light/set-pieces from params). API: `GET /api/maps/:name`.
...
- **Domains (browser-only, touch the scene):** `world.js` (arena + sky/planet/moons/speed field/set-pieces +
