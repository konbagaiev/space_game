...
**Updated:** 2026-08-30 (**The target reacts — hull flash, model punch, camera shudder.**) 2026-08-26 (**The beam is bluer, and its impact flash is now the shared one** — it emits `bulletImpact` like
...
- **Hit feel — the TARGET reacts** (`client/src/hit-fx.js`; tunables + pure seams in
  ...
  **(1) a hull flash** — an emissive wash over the victim's own (per-instance) materials, on **every** ship:
  ...
  enemy hulls), which is accepted — changing a map slot would force a shader recompile (§83).
