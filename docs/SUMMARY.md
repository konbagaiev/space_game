...
- **Level flow** — driven by a DB **level descriptor** (a phase/wave script) played by the client's
  ...
  - **`level-2` — "Level 1" (beginner):** fighters only (3 at a time) → after **6 kills** rocketeers
  ...
  - **`level-3` — "Level 2" (medium):** fighters only until 5 kills → fighters + rocketeers 75/25
  ...
  - **`level-4` — "Level 3" (full fight):** waves of all three enemy types → after 16 kills spawning
  ...
  - **`level-5` — "Level 4" ("Find the pirate base"):** clearly harder — **pirate gunners + rocketeers
    ...
    **Second Boss** (two Advanced pirate cannons + three rockets) → Victory. Its briefing has **no grant
    ...
    reached the `level-5` row (matched by NAME, not by id — DECISIONS §91/§95; the hangar shop was already
...
- **Side missions — on the Main Window's Missions board** (`docs/plans/mission-generator.md` +
  ...
  once the player has reached the `level-5` row (`sideMissionsUnlocked`, resolved by level name, never by a
...
- **Player progress:** `players.current_progress` stores the player's currently-available level — an
  ...
  **`levels.id` is not contiguous:** the idempotent seed's `ON CONFLICT` path burns a `BIGSERIAL` value on
  every boot, so live ids drift (production: 1, 6, 7, 71, 564 for `level-1`..`level-5`). Ordering is
  gap-tolerant, and every progress **gate resolves its threshold by level `name`** via `reachedLevel()` —
  never against a hardcoded id (DECISIONS §95).
...
- **Stash & hangar shop (`stash` table, migration 011 / Postgres bootstrap):** a player inventory keyed by
  ...
  (derived via `reachedLevel(progress, SIDE_MISSIONS_MIN_LEVEL = 'level-5')` — a level-NAME lookup that
...
- **Side missions:** `server/src/missions.js` is a stateless generator (`generateMissions()`) that emits
  ...
  returns them, **gated by `sideMissionsUnlocked`** (progress has reached the `level-5` row, matched by
...
- **Domains (browser-only, touch the scene):** `world.js` (arena + sky/star-system bodies
  ...
  `disposeSpeedField`/`speedFieldLayers`)/set-pieces + `buildMap`), `systemmap-ui.js` (the system-map screen:
  `drawSystemMap`/`openSystemMap`/`mountBaseMenuMap` + the "Start mission?" prompt), `ship-factory.js`
...
- **Level-id drift** — `server/src/levels_drift.test.js` (2): runs against its own throwaway DB
  ...
  **locked** (`GET /missions` → 403 — the exact production bug, where `6 >= 5`/`6 >= 3` opened both), the
  shop opens on reaching `level-3`, the board only on reaching `level-5` (`/missions` → 200), plus
