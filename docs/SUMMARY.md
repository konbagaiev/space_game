...
- **Star system / navigation (roam).** Out of combat the game world is a **to-scale, flyable star system**
  ...
  - **System-map screen (`systemmap-ui.js`).** A top-down overview (star + 4 orbit circles at their
    wall-clock positions, the belt, the base/science/mining markers, one **body-anchor** marker per body you
    must travel to — `listDestinations()` `kind:'planet'`, the star plus every planet except the home one,
    each sitting at that body's own (x,z) — and the player heading). Opened from the
