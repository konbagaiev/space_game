...
- **Session recordings (funnel analytics, `docs/plans/2026-08-03-1246-record-all-sessions.md`):**
  **every live campaign session** (side missions excluded in v1 — their server-generated descriptors
  ...
  `CREATE TABLE`). The client flushes on win/death (`fetch`) and on `pagehide` (`sendBeacon`, best-effort —
  large abandoned traces may exceed the ~64KB beacon cap and drop). `game_version` = the deploy commit,
  stamped **server-side** from `process.env.SENTRY_RELEASE`. Trivial sessions **< 180 ticks (~3 s)** are
  dropped; capture stops appending past **36000 ticks (~10 min)** (server hard-caps at 40000 → 413).
