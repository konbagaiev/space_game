...
- **Rocket detonation** (`spawnRocketBurst`): a rocket blast uses layered fireball spheres + a few sparks +
  the **same soft `spawnShockRing`** as the ship death, but **shrunk and fast**, so it reads as a proper
  explosion rather than one glowing sphere. Its **size, speed and tint are data-driven** from the rocket's
  weapon stats — `blastVisual` (size), `blastTimeScale` (lifetime multiplier; `0.8` = 20% quicker) and
  `blastTint` (color). It **keeps the `sparks` pool** (a deliberately snappy separate effect) + `G.gfx`
  tier gating — distinct from the ship-death burst.
