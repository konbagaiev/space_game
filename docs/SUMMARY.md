...
- **Star system / navigation (roam).** Out of combat the game world is a **to-scale, flyable star system**
  ...
  - **Navigation UI — ONE component, three hosts (`systemmap-ui.js` `mountSystemNav`, DECISIONS §100).**
    ...
    the map.
    - **Objects (`listSystemObjects()`, 10).** The **star and all four planets are first-class, selectable
      destinations**, listed and marked exactly like the **home station**, the **research station** and the
      ...
      **star 14px**, **planets 10px**, everything else (stations, outposts) **4px** — so the celestial
      bodies read as bodies against the man-made anchors; the selection ring, lock ring and name label
      all offset from that radius.
...
- **Mission set-pieces (procedural decor).** The descriptor can carry a **`setpieces`** array — large
  ...
  - **`base-station`** — the **return-to-base target** at `(-10,-10)`, just up-left of the arena center
    (14 u off it, so the origin-spawning ship IS framed over the station). A `.glb`-backed set-piece (`base_station_combat`, CC-BY 4.0 "Low Poly space station." by MisterH)
