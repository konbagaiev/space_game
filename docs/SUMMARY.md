...
- Camera: nearly vertical, rigidly attached to the player, does not rotate. The fixed offset
  ...
  angle — zoom never changes the angle, FOV, or camera type. **Zoom also slides the fog** (`applyZoom`,
...
- **Main Window (the between-battles / landing screen; was the "Hangar")** — `#mainwin`, a **fixed
  ...
  **Character** is a real screen (see **Character progression** below); **Map / Craft** are **stub
  ...
  - **Staged campaign-briefing reveal (L2/L3)** (`docs/plans/2026-07-05-1641-briefing-staged-reveal.md`):
    ...
    **Take off** button (`#mw-go`) is hidden (`.briefing-hide-go`, `visibility:hidden`, so nothing reflows)
...
- **Star system / navigation (roam).** Out of combat the game world is a **to-scale, flyable star system**
  ...
  - **System-map screen (`systemmap-ui.js`).** A top-down overview (star + 4 orbit circles at their
    wall-clock positions, the belt, the base/science/mining markers, one **planet-anchor** marker per
    non-home planet — `listDestinations()` `kind:'planet'`, the reachable transfer point autopilot actually
    flies to, since the planet itself is permanently distant backdrop — and the player heading). Opened from the
...
- **Star-system backdrop bodies (`system-map.js` geometry + `world.js`
  `buildSystemBodies`/`updateSystemBodies`).** A central **star + 4 planets + the home planet's 2 moons**,
  drawn as **real 3D spheres at FIXED positions** in the sky scene (DECISIONS §98). Each body's **direction is
  its true bearing from the origin, resolved ONCE at build** (`bodySkyDir`; `SYSTEM.elev` tilts the layout
  down into the near-top-down camera's sky band, and a body may override it with its own `elev`); its
  **distance and radius are art-directed** (`dist`/`size` — star 54 @ 700 u ≈ 1.2× a planet; home planet 58 @
  430 u; the others 30–36 @ 560–680 u), because the true 45 000 u distances are 50× the camera's 900 u far
  plane. Per frame `updateSystemBodies` moves only the **group** the bodies sit in, to
  `camera − skyParallax(player)` — the projection of a system compressed by `SYSTEM.parallax` (0.02). So you
  get real perspective, real depth ordering and **gentle differential parallax** (the home planet slides
  faster than the star), bodies that **never jump**, and — since `skyParallax` saturates at `parallaxMax`
  (90 u) — bodies that are **permanently distant**: you can never reach or loom up to one, the home planet
  included (it stays a 430 u backdrop at the base; there is no "home is near" scale-up). **"To-scale" means
  the travel distances (world coords) only.** Planet 2 is the ocean base planet, **pinned to the world
  origin** (system-map.js), so the base/set-pieces/mission centers stay origin-relative; because it therefore
  has no bearing of its own, `homeDir` art-directs where it hangs. Its **moons** orbit in sky space at radii
  kept clear of its limb (`moonClearance` > 0, unit-asserted at every angle), animated off wall-clock.
  The descriptor's `system` block is **merged into** the client `SYSTEM` at build (`applySystemSpec`), so
  renderer / map screen / `?roam` tunables share one object. Pure view layer → replay-neutral.
  *Known limit:* the camera's fixed near-top-down orientation means a body whose true bearing points behind
  the ship falls off the bottom of the screen, and two planets sharing a bearing that day overlap.
