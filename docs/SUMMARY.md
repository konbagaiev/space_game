...
- **Trace format v2** (`{version:2, kind:'input-replay', id, level, seed, dt, shipId, tickCount,
  ...
  count that works on either shape.
...
- **Determinism isolation (load-bearing) — the seeded stream is OPT-IN (`client/src/sim-random.js`).**
  ...
  **bit-for-bit** (a rounded-position state hash) regardless of frame rate, audio state, or model-load timing.
  Accepted cosmetic cost: asteroid/decor layout now varies between two playbacks of the same trace.
  See DECISIONS §73.
...
- **Shield-hit FX (`shield-fx.js`):** a cosmetic translucent **bubble** (a `ShaderMaterial` sphere of radius
  ...
  *world* radius with `mesh.scale` folded in ≈ 2.2 small pirate / 4.0 medium / 6.6 boss) since the enemy
...
- **Asset pipeline** (`docs/plans/ship-model-pipeline.md` + `audio-sample-pipeline.md`): repo-root `npm run
  ...
  **Material flattening (combat only, `scripts/assets-flatten.mjs`, DECISIONS §77).** A model opts in with
  ...
  `catalog_seed.js`**, **and the `level-1` descriptor's `introTrace`** exist on S3 — the deploy guard). **No model binaries in git** (S3 canonical; the pre-load fallback is a
...
- **Main Window (the between-battles / landing screen; was the "Hangar")** — `#mainwin`, a **fixed
  ...
  **Below**: a 3-column grid — **left menu** (`#mw-menu`: **Character / Missions / Loadout / Map / Craft** —
  ...
  is also `max-width: 200px`, wrapping rather than overflowing). The **Missions** item opens a
  central **board** (`#mw-mission-board`, `renderMissionsBoard`) of cards — the **campaign** ("Main
  operation") card plus, once the side-mission board unlocks (after "Level 3" — DECISIONS §91), the
  **three side-mission** cards — each with **Take / Defer / Set active** buttons + **Active/Taken**
  badges (the old left mission sublist + caret are gone). Clicking a card shows its briefing in the
  detail area below (**only the description scrolls**, `#mw-mission-desc`). **Take-off flies the ACTIVE
  mission** (`activeMissionId`; the campaign when none is active), and its title shows on the button
  ...
  **right context panel** (`#loadout-panel`): the equipped item's info + **its 3D model** + **Remove**, the
  ...
  `renderShopDetail` in shop.js). The Loadout widens the right column a touch (30%) for the model. The layout keeps the Main Window's normal
  **three-column grid** (menu | work | 25%): the ship+slots fill the center work zone, and the context
  **panel lives in the right column** (`#loadout-panel` inside `#mw-ship-col`, same width ratio as the
  missions ship preview, **borderless — no boxed rectangle**); `#mainwin.bay-open` hides the right-column
  ship preview (`#mw-ship`) and reveals the panel, `#ship-stats` stays above it. **Before the shop unlocks**
...
- **Star system / navigation (roam).** Out of combat the game world is a **to-scale, flyable star system**
  ...
  - **Uncapped-outside-zones travel.** The player speed cap (`PLAYER_MAX_SPEED` 30) is **lifted in roam
    whenever the ship is outside every activity zone** (`capLifted({roam,inZone})`, pure/tested), so §2 inertia
    grows speed to cross the system; inside a zone (`ZONE_RADIUS` ≈ ARENA 360 around the base/science/near-mining
    anchors + the active mission center) it re-caps. **`capLifted` is false whenever `G.roam` is false** — the
    invariant that keeps every recorded/campaign replay byte-identical. The OOB warn/warp-back is disabled in roam.
  - **System-map screen (`systemmap-ui.js`).** A top-down overview (star + 4 orbit circles at their
    wall-clock positions, the belt, the base/science/mining markers, the player heading). Opened from the
    base-menu **Map** section (a real section now) — **"Launch into system"** → `enterRoam(null)` (free
    cruise), **"Fly here"** per reachable mission → `enterRoam({pos, missionId})` — and, **out of combat only**,
    by a **tap on the mini-map** (`#minimap`); during a live fight the mini-map stays the battle radar. The
    overlay **freezes the game via a raw loop-skip on `G.mapOpen`** (NOT `setPaused`, which would stack the
    Paused UI). Picking a destination re-routes the autopilot; **"Return to hangar"** ends roam back to the base menu.
...
- **Level flow** — driven by a DB **level descriptor** (a phase/wave script) played by the client's
  ...
  - **`level-5` (id 5) — "Level 4" ("Find the pirate base"):** clearly harder — **pirate gunners + rocketeers
    ...
    **Second Boss** (two Advanced pirate cannons + three rockets) → Victory. Its briefing has **no grant
    actions**, but now **announces the side-mission board** — which unlocks here by a progress gate
    (`current_progress >= 5`, DECISIONS §91; the hangar shop was already opened back at "Level 2" — see
...
- **Shop & stash (the "spend" side)** — once the player **clears the first mission** (shop unlocks early,
  ...
  **server-authoritative + transactional**: **buy** (credits down → item into stash), **sell** (stash item
  ...
  single unit (no dialog). A **live ship-stats panel** shows **HP / acceleration /
...
- **Side missions — on the Main Window's Missions board** (`docs/plans/mission-generator.md` +
  `2026-08-08-base-menu-redesign.md`). Unlocked **after clearing "Level 3"** (a progress gate,
  `current_progress >= 5`, `sideMissionsUnlocked`), **separate from the shop** which opens earlier
  (DECISIONS §91). They render as **cards** on the central mission board (below the campaign card), each
...
- **Speed-field (replaces the old origin-ring asteroids; `speed-field.js` pure seam + `world.js`
  `makeSpeedField`/`updateSpeedField`).** A small set of **2-3 THREE.Points layers** (~500 total, clamped
  200-600 from the descriptor `asteroids.count`) kept in a box AROUND the player; each point **wraps back to
  the opposite side** as the player crosses the box edge (pure `wrapCoord`, node-tested), so the field
  streams past and sells speed while roaming. Slower/farther layers wrap against a scaled player delta
  (parallax). Additive round dots, **procedural canvas texture (no asset)**, fog off; color/density from the
  descriptor. Updated every frame in `settleView` → **view layer, zero sim RNG, replay-neutral.** A
  velocity-stretch/warp-streak shader hook is left as a TODO (out of scope). (DECISIONS §71 superseded by §95.)
- **Star-system backdrop bodies (`system-map.js` geometry + `world.js`
  `buildSystemBodies`/`updateSystemBodies`).** A central **star + 4 planets** rendered as **bearing-projected
  sky billboards**: same sky-scene family as the old planet, at a **fixed billboard distance** (constant
  apparent size; the star ~1.2× a planet). Each frame `updateSystemBodies` re-projects every body by its
  **true bearing from the player to the body's real world position** (lifted above the horizon by `elev` for
  the near-top-down camera) — fly toward a planet and it comes forward while the previous one recedes; pass
  it and its bearing flips; the star drifts across the sky. **"To-scale" means the travel distances (world
  coords) only**; body apparent sizes stay readable. Planet 2 is the ocean base planet, **pinned to the world
  origin** (system-map.js), so the base/set-pieces/mission centers stay origin-relative. The old standalone
  moons are dropped. Pure view layer → replay-neutral.
...
- **The whole scene is data-driven:** it's described by a JSON **map descriptor** in the DB (`maps`
  ...
  (the `system` block → star + 4 planets, `asteroids` → speed-field, stars/sky-light/set-pieces from params;
  the client's `SYSTEM` constant in `system-map.js` is the fallback + the source of truth for body POSITIONS).
...
- **Stash & hangar shop (`stash` table, migration 011 / Postgres bootstrap):** a player inventory keyed by
  ...
  advance still a fallback; it also **backfills the basic gun (id 1)** into the stash. A **one-time boot
  backfill** (`migrate()`, idempotent) retroactively opens the shop + seeds the basic gun for existing
  players already past the first flight (`current_progress >= 3`). `replaceWeapon` briefing actions also
  ...
  (derived: `current_progress >= SIDE_MISSIONS_MIN_PROGRESS` = 5), **`launchable`**,
...
- **Side missions:** `server/src/missions.js` is a stateless generator (`generateMissions()`) that emits
  ...
  returns them, **gated by `sideMissionsUnlocked`** (`current_progress >= 5`; 403 until the board opens —
  after clearing "Level 3", DECISIONS §91 — decoupled from the shop, which opens earlier); the response
...
- **Admin dashboard (`GET /admin`, `server/src/admin.js`):** a private, **server-rendered** HTML page
  ...
  **progress**, credits, games_played, referrer, **device**) plus per-player aggregates from `games`
  ...
  `level-1`..`level-5` is off by one and is never shown) plus a CSS bar and an `n/N` fraction, with a **✔**
...
- **Domains (browser-only, touch the scene):** `world.js` (arena + sky/star-system bodies
  (`buildSystemBodies`/`updateSystemBodies`)/speed-field (`makeSpeedField`/`updateSpeedField`)/set-pieces +
  `buildMap`), `systemmap-ui.js` (the system-map screen: `drawSystemMap`/`openSystemMap`/`mountBaseMenuMap` +
  the "Start mission?" prompt), `ship-factory.js` (`makeShip`/`applyShipModel` + `gltfLoader`), `projectiles.js`
  ...
  restart (roam-aware) + `settleView` (camera + system-body bearings + speed-field)), `spawn-timing.js` (the pure enemy-spawn stagger gate
...
- **Between-battles UI:** `shop.js` (hangar shop + stash + live ship-stats bar; a leaf the Main Window
  ...
  — `showMain`/`selectMenu`/mission board/`launchCampaign`/`launchMission`/`refreshMissions`/`enterRoam` (the
  one roam entry) + the real Map section + the mission-arrival `G.onMissionArrival` handler + the ship-
...
- **Client logic** — `client/src/*.test.js`: drive derivation (engine + mass, incl. the grab slot + the
  ...
  **character progression** (`components.test.js` `skillEffects`: empty=identity / one step per point /
...
- **Backend API** — `server/src/server.test.js` (52): register / record game + credit banking / history /
  ...
  **hangar shop/stash** (lock before the first flight, unlock on reaching "Level 2" (id 3) + basic-gun
  backfill, plus the one-time existing-player backfill migration, buy/sell/equip/
