...
- **Shield:** a regenerating damage buffer carried on the **starter ship** (`components.shield = 31`) and
  **buyable**. On the player entity it seeds `_shieldValue` (starts full = `capacity`) + `_shieldRechargeAccum`.
  ...
  tiers will matter).
- **Shield-hit FX (`shield-fx.js`):** a cosmetic translucent **bubble** (a `ShaderMaterial` sphere, radius 4)
  drawn around the player ship while a shield is equipped. Idle it shows only a faint Fresnel rim; on every
...
- **Visual model:** the **ship visual-model rendering lives in `client/src/ship-factory.js`** — `makeShip` builds a
  ...
  **Player damage routes through the shield first.** Every incoming player-damage site — enemy bullet hits
  ...
  (swept hull test + `applyPlayerDamage`, returning `{ hit, damageResult, remove }`); it is unit-tested and
  side-effect-/RNG-free (so record/playback stays deterministic), while the scene.remove / hit-flash /
  shield-ripple / SFX and the range-based bullet culling stay inline in `sim.update()`. `applyPlayerDamage`
  absorbs into the shield (`absorbDamage`)
