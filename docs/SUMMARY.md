...
**Updated:** 2026-07-04 (**Multi-sphere ship hitboxes** — each real-model ship now collides as ~4-8
auto-fit hull spheres (broad-phase enclosing sphere → per-sphere narrow-phase) instead of one fat bubble;
spheres chained along the hull's longest axis with capped radii so they hug (not balloon); generated by
`assets:hitspheres` into `model.hitSpheres`/`broadR`, wired at all four bullet/rocket↔ship sites plus the
rocket blast-damage loop (player included, fixing the hardcoded 2.6 and the zero-damage rocket bug), with a
`?hitspheres` wireframe overlay.)
(**Enemy HP bar clears the model** — the over-enemy health bar now pins its
...
- **Visual model:** the **ship visual-model rendering lives in `client/src/ship-factory.js`** — `makeShip` builds a
  ...
  **`model.hitSpheres` + `model.broadR` — the auto-fit collision hitbox.** Instead of a single fat sphere,
  each real-model ship carries **~4-8 spheres** fitted to its actual hull by the `assets:hitspheres`
  pipeline step (spheres chained along the hull's **longest horizontal axis**, each radius hugging the
  perpendicular cross-section and capped so it can't bulge past the hull, + up to 2 thin wing spheres;
  radii inflated by `HITSPHERE_PAD` 1.1), stored in the **same group-local noseZ frame** as
  `userData.noseZ` (after auto-scale to `SHIP_MODEL_LEN` 3.4 + recenter + `yaw`). `broadR` is the enclosing
  radius (~1.6-2.1, near the model half-length). Do **not** hand-author these — regenerate with
  `npm run assets:hitspheres` (writes them into the seed's `model:{}` blocks via a marker-delimited
  idempotent edit, verified by a seed round-trip; `HITSPHERES_DEBUG=1` prints the fit). Collision
  (`client/src/collision.js`) is broad-phase (one `broadR × mesh.scale.x` sphere at `mesh.position`) →
  narrow-phase (each sphere transformed by `mesh.matrixWorld`, radius `r × mesh.scale.x`, ignoring the
  cosmetic bank roll). Ships with no `hitSpheres` (primitive/cone fallbacks) keep the legacy single
  `2.6 × sizeScale` sphere. All four bullet/rocket↔ship sites use `pointHitsShip` — including the **player**
  (fixing the old hardcoded `2.6` and the player↔rocket test that ignored ship size); the rocket's
  `detonateR` becomes the hit `pad` — so it's now a small **proximity fuse to the hull surface**
  (retuned down to ~1.0–1.2, from the old ~3.2–3.5 which measured to the ship *center*; leaving it large
  made rockets detonate a ship-length away). **Rocket blast (AoE) damage is hull-relative too**
  (`detonateRocket` → `pointHitsShip(ship, pos, blastR)`), matching the detonation trigger — a
  center-distance test used to miss because the detonation point sits off-center on a hull sphere.
  `e.radius` (`2.6 × scale`) is kept **only** as the over-enemy health-bar / marker anchor. Dev-only
  `?hitspheres` draws the wireframe spheres over every ship for eyeballing. An optional
...
- **Wing-bank on turn:** every ship (player + enemies) **rolls its wings into a turn**, a smooth tilt
  ...
  roll (aim/forward use `heading`; collisions use the multi-sphere hitbox, whose spheres ride `mesh.matrixWorld`
