...
**Updated:** 2026-07-04 (**Multi-sphere ship hitboxes** — each real-model ship now collides as ~5-10
auto-fit hull spheres (broad-phase enclosing sphere → per-sphere narrow-phase) instead of one fat bubble;
generated by `assets:hitspheres` into `model.hitSpheres`/`broadR`, wired at all four bullet/rocket↔ship
sites (player included, fixing the hardcoded 2.6), with a `?hitspheres` wireframe overlay.)
...
- **Visual model:** the **ship visual-model rendering lives in `client/src/ship-factory.js`** — `makeShip` builds a
  ...
  **`model.hitSpheres` + `model.broadR` — the auto-fit collision hitbox.** Instead of a single fat sphere,
  each real-model ship carries **~5-10 spheres** fitted to its actual hull by the `assets:hitspheres`
  pipeline step (axial Z-slices + up to 2 lateral wing spheres, radii inflated by `HITSPHERE_PAD` 1.1),
  stored in the **same group-local noseZ frame** as `userData.noseZ` (after auto-scale to `SHIP_MODEL_LEN`
  3.4 + recenter + `yaw`). `broadR` is the enclosing radius. Do **not** hand-author these — regenerate with
  `npm run assets:hitspheres` (writes them into the seed's `model:{}` blocks via a marker-delimited
  idempotent edit, verified by a seed round-trip). Collision (`client/src/collision.js`) is broad-phase
  (one `broadR × mesh.scale.x` sphere at `mesh.position`) → narrow-phase (each sphere transformed by
  `mesh.matrixWorld`, radius `r × mesh.scale.x`, ignoring the cosmetic bank roll). Ships with no
  `hitSpheres` (primitive/cone fallbacks) keep the legacy single `2.6 × sizeScale` sphere. All four
  bullet/rocket↔ship sites use `pointHitsShip` — including the **player** (fixing the old hardcoded `2.6`
  and the player↔rocket test that ignored ship size); the rocket's `detonateR` becomes the hit `pad`.
