...
- **Perf overlay** at the top center: FPS, frame time (ms), draw calls, triangles, and a **ship-speed
  ...
  **Touch is non-sticky** (`evalDevForDevice` in `dev.js`, keyed on `Device.input === 'touch'`): on a
  phone/tablet the stored flag is **ignored and never written**, so dev diagnostics are on **only** when an
  explicit truthy `?dev` is in *this* load's URL — a one-off `?dev` telemetry visit can't leave the FPS/tris
  overlay stuck on in normal play (a phone has no ergonomic way to clear it). Desktop keeps the sticky flag.
...
- **Determinism isolation (load-bearing) — the seeded stream is OPT-IN (`client/src/sim-random.js`).**
  ...
  arm, `?bench` bootstrap + per-trace), `seedSim(null)` clears it back to the native `Math.random` (live play;
  called from `finishIntro()` and at the end of `stopRecordSession()`), `isSimSeeded()` reports the state.
...
- **Shield:** a regenerating damage buffer carried on the **starter ship** (`components.shield = 31`) and
  **buyable**. On the player entity it seeds `_shieldValue` (starts full = `capacity`) + `_shieldRechargeAccum`.
  ...
  never recharges), and `applyPlayerDamage(player, dmg)` (the shield-first damage router; returns
  `{ absorbed, broke }` for the hit FX). The recharge ticks each sim frame (freezes on pause; refills to full
...
- **Visual model:** the **ship visual-model rendering lives in `client/src/ship-factory.js`** — `makeShip` builds a
  ...
  **Player damage routes through the shield first.** Every incoming player-damage site — enemy bullet hits
  (`sim.js`) and the rocket blast (`projectiles.js`) — calls `applyPlayerDamage(player, dmg)` (in
  `components.js`, alongside `absorbDamage`/`shieldRecharge`). The enemy-bullet→player hit resolution goes
  ...
  the sphere surface**, not the hull inside it. `applyPlayerDamage` absorbs into the shield (`absorbDamage`)
...
- **Mission set-pieces (procedural decor).** The descriptor can carry a **`setpieces`** array — large
  ...
  - **`freighter`** — the **first `.glb`-backed set-piece** (the base-station and asteroid-field packs
    ...
    **fiery exhaust**, now the **shared GPU/baked-texture, shader-driven axis-aligned plume**
    ...
    `fx.setOrigin`. The plume ships **two selectable looks** — **(a)** point-glow (default, reproduces the old
    hot→orange→red silhouette) / **(b)** a noise-scroll flame — switched **globally** (freighter + every ship
...
- Effects: a bullet hit-flash at the impact point, **keyed off the weapon `class`** via the client
  ...
  small `maxScale 2` flash; color unchanged); a glowing engine exhaust plume on **every ship** (player and
  enemies) — the **same shared GPU/baked-texture plume as the freighter** (`exhaust-fx.js`
  `attachShipExhaust`), **rigidly attached to the ship mesh and always straight** along the aft `-Z`
  (the old curved position-history particle trail is gone — a deliberate trade-off, DECISIONS §74). It is a
  **fixed-cost render object, not a growing particle pool**: `emitExhaust` (called from the player/enemy
  thrust sites) just lazily attaches the plume (cached on `mesh.userData.exhaustPlume`, count tier-scaled
  once at attach) and flags `throttleTarget = 1`; `updateShipExhaust(dt)` (in `sim.update`) smoothly decays
  each plume's `throttle` toward the target then zeroes it, so a ship that stops thrusting **fades out**
  (the shader multiplies alpha + point size by the throttle). The plume is derived from the engine's single
  `exhaust.color` (`derivePalette` → hot/mid/end) and **honors the same global (a) point-glow / (b) flame
  look toggle as the freighter**. The old `trail` particle pool (`state.js`) and `spawnTrail` are **removed**;
...
- **Kinetic fire visual** (`bolt-fx.js`): kinetic bullets render as a **travel-aligned additive glow
  bolt** (a hot-white core + soft rim from one shared, once-uploaded glow texture) laid flat on the combat
  plane, instead of the old flat opaque sphere; each shot also pops a quick **muzzle flash** at the barrel
  (tinted by the weapon color, reusing the micro-explosion flash). One draw call per shot (same cost as
  the sphere it replaces); other weapon classes keep the plain sphere. Replay-safe (bolt orientation is
  derived from the constant bullet velocity — no RNG). `spawnBullet` branches on `weapon.class`.
- **Muzzle / exhaust spawn from the model's real bounds.** Bullets/rockets leave the **nose** and the
  ...
  scale (so it tracks spawn-grow), and `attachShipExhaust` sets the plume's local origin to `(0,0,tailZ)`
  (parented to the mesh, so it follows scale + yaw for free). Replaces the old fixed `fwd*3` / `fwd*-2.6`
...
- **Ship destruction** (`spawnShipExplosion`): a destroyed ship bursts in a **flipbook (sprite-sheet)
  ...
  animation (~0.78 s), replacing the old stack of 4 additive fireball spheres — plus a radial spray of
  sparks and an expanding shockwave ring. **One shared sprite-sheet texture uploaded once for the session
  + one draw call per blast**, so it stays on at every graphics tier (the mobile-friendly path, DECISIONS
  §23) and is replay-safe (no sim RNG). **Sized to the ship** (`sizeScale`); the **sparks + ring are
  tinted by the engine's exhaust color** (player cyan-blue, enemies orange) while the flipbook carries its
  own baked fire colors. Used on enemy and player death. An enemy death also spawns a floating `+xx`
  credit popup at the kill site (see the HUD "Kill credit popups").
...
- **Monitoring / observability** (`docs/plans/monitoring.md`):
  ...
  - **Client perf samples (`?dev` monitor).** `perf_samples` table (migration 015 / Postgres bootstrap):
    ...
    recording. **Off — zero overhead — without `?dev`.** (`devPerf` is gated on `isDev()`, so on **touch** it
    too is non-sticky — a phone reports only while an explicit `?dev` is in the URL this load, not forever
    after one visit; see the Perf-overlay bullet.) Write-only over HTTP (no public read); analyze
...
- **Input-replay:** `replay.js` — the pure, DOM/engine-free half of `?record`/`?playback` (URL-flag parsing,
  the `{seed,dt,shipId,level,ticks}` trace shape, per-tick input snapshot/apply, validate, the
...
- **Client logic** — `client/src/*.test.js`: drive derivation (engine + mass, incl. the grab slot + the
  ...
  refill-to-full-at-rechargeSec / no-overshoot), **loot drops**
  ...
  **audio settings** (`clamp01`, `loadAudioSettings`/`saveAudioSettings` round-trip + defaults + garbage
  ...
  counts/resets, `stalled()` trips exactly at `CUTSCENE_STALL_TICKS`, `teardown()` clears every field).
