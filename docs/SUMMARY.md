...
- **Perf overlay** at the top center: FPS, frame time (ms), draw calls, triangles, and a **ship-speed
  ...
  **Touch is non-sticky** (`evalDevForDevice` in `dev.js`, keyed on `Device.input === 'touch'`): on a
  ...
  record/sliders panel) are **never built on touch** (`Device.input !== 'touch'` guards in `main.js`
  bootstrap) — they're mouse-only tools; the perf overlay still shows under an explicit `?dev` on touch, just
  not those panels.
...
- **Visual / e2e** — `client/visual/` (Playwright headless, **not in CI**): boots the real game and
  ...
  **audio** (the settings gear opens the audio modal; the Master slider + Music toggle reach the engine and
  ...
  cutscene; needs `npm run assets:pull` first, since the trace is a gitignored S3 asset).
