...
- **Between-level briefings** — a level descriptor can carry an optional **`briefing`** (`{ textKey,
  ...
  Gun** (5), **and now also opens the hangar shop + side missions** (`unlockShop`) — the shop unlocks right
...
- **Level flow** — driven by a DB **level descriptor** (a phase/wave script) played by the client's
  ...
  - **`level-3` (id 3) — "Level 2" (medium):** fighters only until 5 kills → fighters + rocketeers 75/25
    ...
    hangar shop + side missions** — `unlockShop`) + repair-drone drop.
  ...
  - **`level-5` (id 5) — "Level 4" ("Find the pirate base"):** clearly harder — **pirate gunners + rocketeers
    ...
    **Second Boss** (two Advanced pirate cannons + three rockets) → Victory. Its briefing is **text-only**
    now (the hangar shop + side missions were already opened back at "Level 2" — see Between-level
    briefings); its victory sets up the planned L5 ("Storm the pirate base"). Currently the final level.
...
- **Side missions:** `server/src/missions.js` is a stateless generator (`generateMissions()`) that emits
  ...
  returns them, **gated by `shop_unlocked`** (403 until the shop unlocks — right after the first flight, on
  reaching "Level 2"). Each descriptor carries
