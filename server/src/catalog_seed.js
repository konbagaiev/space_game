...
// The ship WEIGHT-CLASS table. It lives under client/ because the browser bundle never imports from
// server/ (DECISIONS §136) and sim-host.js already imports across the same boundary; the Dockerfile copies
// client/ into the image, so this resolves in production too.
...
// Menu-only item icons shared by a whole component FAMILY. Components are never rendered in the combat
// scene (there the model belongs to the ship), so they wire only `modelUrlHigh` — the lazy-loaded
// CloudFront hangar glb — exactly like the Repair drone (12) and the Machine Gun weapon (5).
// One model per family for now; per-tier distinct models are a deliberate future iteration, not an
// oversight — swapping one in means editing the constant here, nothing else.
// The ENGINE glb is SKINNED and carries a looping flame animation ("Flame startAction", 6 s) that the item
// viewer plays (client/src/model-viewer.js). Its `Fire` material is alpha-blended.
const ENGINE_MODEL = 'https://d1843uwjdjg4vs.cloudfront.net/ships-hangar/maneuver_thruster_hangar.7445cd77.glb';
const THRUSTER_MODEL = 'https://d1843uwjdjg4vs.cloudfront.net/ships-hangar/engine_thruster_hangar.d85b7e72.glb';
// Shared item-preview presentation per family (yaw/pitch/scale, read by itemModelCfg). The viewer
// normalizes the longest axis, so `scale` is a relative nudge, not an absolute size.
// `pitch` (Math.PI / 2) lays the engine nozzle on its side — it models standing upright, and the preview
// already spins about the VERTICAL axis, so a `yaw` would only shift the spin phase, never tip it over.
// `scale` 0.75: the flame is fitted at its BIND pose, but the clip scales it well past that — at 1.0 the
// plume grew straight through the edge of the preview canvas. Leaves headroom for the animation's extent.
...
export const COMPONENTS = [
  ...
  { id: 5, name: 'Basic engine', type: 'engine', weight: 10, price: 500,
    modelUrlHigh: ENGINE_MODEL, // menu-only item icon (shared by the family)
    stats: { power: 15, maxSpeed: 0, exhaust: { color: 0x6fd0ff, speed: 12, life: 0.55, size: 0.5, spread: 0.35 } , model: ENGINE_MODEL_CFG } }, // starter gear: cheap
  { id: 6, name: 'Scout engine', type: 'engine', weight: 6, price: 250,
    modelUrlHigh: ENGINE_MODEL, // menu-only item icon (shared by the family)
    stats: { power: 19, maxSpeed: 10.5, exhaust: { color: 0xff8a5a, speed: 10, life: 0.4, size: 0.4, spread: 0.3 }, buyable: false , model: ENGINE_MODEL_CFG } }, // enemy gear: resale-only
  { id: 7, name: 'Boss engine', type: 'engine', weight: 50,
    modelUrlHigh: ENGINE_MODEL, // menu-only item icon (shared by the family)
    stats: { power: 29, maxSpeed: 10.4, exhaust: { color: 0xff5a3a, speed: 10, life: 0.6, size: 0.9, spread: 0.45 } , model: ENGINE_MODEL_CFG } }, // boss buff: maxSpeed 8 → 10.4 (+30%)
  { id: 8, name: 'Basic thrusters', type: 'thruster', weight: 4, price: 400,
    modelUrlHigh: THRUSTER_MODEL, // menu-only item icon (shared by the family)
    stats: { power: 2.0 , model: THRUSTER_MODEL_CFG } }, // starter gear: cheap
  { id: 9, name: 'Scout thrusters', type: 'thruster', weight: 3, price: 200,
    modelUrlHigh: THRUSTER_MODEL, // menu-only item icon (shared by the family)
    stats: { power: 1.6, buyable: false , model: THRUSTER_MODEL_CFG } }, // enemy gear: resale-only
  { id: 10, name: 'Medium thrusters', type: 'thruster', weight: 8,
    modelUrlHigh: THRUSTER_MODEL, // menu-only item icon (shared by the family)
    stats: { power: 0.63 , model: THRUSTER_MODEL_CFG } }, // sluggish (turn ~0.35)
  { id: 11, name: 'Boss thrusters', type: 'thruster', weight: 20,
    modelUrlHigh: THRUSTER_MODEL, // menu-only item icon (shared by the family)
    stats: { power: 1.66 , model: THRUSTER_MODEL_CFG } }, // turn ~0.42 = 1.2× medium
  ...
  { id: 23, name: 'Pirate engine', type: 'engine', weight: 6, price: 400,
    modelUrlHigh: ENGINE_MODEL, // menu-only item icon (shared by the family)
    stats: { power: 19, maxSpeed: 15.75, exhaust: { color: 0xff6a4a, speed: 10, life: 0.4, size: 0.4, spread: 0.3 }, buyable: false , model: ENGINE_MODEL_CFG } }, // maxSpeed 10.5 × 1.5; same accel as Scout; enemy gear: resale-only
  ...
  { id: 25, name: 'Pirate medium thruster', type: 'thruster', weight: 8, price: 350,
    modelUrlHigh: THRUSTER_MODEL, // menu-only item icon (shared by the family)
    stats: { power: 1.25, buyable: false , model: THRUSTER_MODEL_CFG } },          // ~+30% net turn vs Medium (0.63) once mass-scaled; enemy gear: resale-only
  // Second Boss (550 HP, speed/accel/turn ~+30% vs the first boss):
  { id: 26, name: 'Second-boss engine', type: 'engine', weight: 50, price: 1500,
    modelUrlHigh: ENGINE_MODEL, // menu-only item icon (shared by the family)
    stats: { power: 45, maxSpeed: 14.3, exhaust: { color: 0xff3a2a, speed: 11, life: 0.6, size: 0.95, spread: 0.45 }, buyable: false , model: ENGINE_MODEL_CFG } }, // boss buff: maxSpeed 11 → 14.3 (+30%); enemy gear: resale-only
  { id: 27, name: 'Second-boss thruster', type: 'thruster', weight: 20, price: 900,
    modelUrlHigh: THRUSTER_MODEL, // menu-only item icon (shared by the family)
    stats: { power: 2.7, buyable: false , model: THRUSTER_MODEL_CFG } },            // boss 1.66 bumped for ~+30% net turn; enemy gear: resale-only
  ...
  // --- THE 50°/s PIRATE THRUSTERS (maintainer, 2026-08-25, after flying the build): "make it 50 for
  // everyone except those in the intro". `turnRate` is DERIVED, never a field
  // (`deriveDrive`: turnRate = thruster.power × REFERENCE_MASS / mass), so the only way to give a ship its
  // own turn rate is to give it its own thruster — the same reason Pirate medium (25) and Second-boss (27)
  // exist.
  //
  // WHY TWO ROWS AND NOT ONE. A thruster's power yields 50°/s at exactly ONE mass, because the mass divides
  // into it. So "50 for everyone" needs one row PER MASS TIER, not one row: power = 0.872665 × mass / 50.
  //     mass 25 → 0.4363   (pirate gunner)
  //     mass 31 → 0.5410   (pirate lancer, advanced rocket pirate)
  // A future fighter at some other mass needs a THIRD row; it cannot reuse these and land on 50.
  //
  // BOTH ARE WEIGHT 3, THE SAME AS THE SCOUT THRUSTERS (9) THEY REPLACE, and that is load-bearing: it keeps
  // every carrier's mass exactly where it was, so ACCELERATION IS UNTOUCHED (acceleration comes off the
  // ENGINE and is scaled by the same mass). The instruction was to slow the TURN, not the ship. Change a
  // weight here and the power must be re-derived with it.
  //
  ...
  { id: 32, name: 'Pirate fighter thruster', type: 'thruster', weight: 3, price: 200,
    modelUrlHigh: THRUSTER_MODEL, // menu-only item icon (shared by the family)
    stats: { power: 0.541, buyable: false , model: THRUSTER_MODEL_CFG } },   // → 50°/s at mass 31 (lancer, advanced rocket pirate); enemy gear: resale-only
  { id: 33, name: 'Pirate skirmisher thruster', type: 'thruster', weight: 3, price: 200,
    modelUrlHigh: THRUSTER_MODEL, // menu-only item icon (shared by the family)
    stats: { power: 0.4363, buyable: false , model: THRUSTER_MODEL_CFG } },  // → 50°/s at mass 25 (pirate gunner); enemy gear: resale-only
];
...
//   A beam row may also be ENEMY GEAR (`buyable: false`, no `minLevel`) — see id 13, the pirate lancer's.
...
export const WEAPONS = [
  ...
  },
  {
    ...
    id: 7, name: 'Heavy Machine Gun', type: 'bullet', price: 6000, stats: { // strong all-rounder: med damage, high rate of fire
      power: 12, projectileSpeed: 48, maxRange: 100, fireCooldown: 0.12, weight: 15, projectileColor: 0xb46bff, class: 'kinetic',
  ...
  //
  // 45 damage on a 3.0 s cycle (1.0 charge + 2.0 cooldown) = 15 sustained DPS — BELOW the pirate machine
  // gun's 16.7 and above the advanced pirate cannon's 10. It is still 2.25x the biggest single hit that
  // exists (the 20-damage pirate rocket, which is dodgeable AND shootable-down while a hitscan beam is
  // neither), and that is the trade: a big, rare, announced hit rather than a stream.
  //
  // THE COOLDOWN IS 2.0 BECAUSE THE MAINTAINER FLEW IT (2026-08-25). The first pass shipped 0.5 — a 1.5 s
  // cycle and 30 DPS, the highest sustained enemy DPS in the game — and in play that read as relentless. He
  // asked for "1 second charge, 2 seconds cooldown" directly. DO NOT restore the shorter cooldown to "fix"
  // the DPS: the pressure this weapon is meant to apply is the TELEGRAPH — a full second of three drawn
  // lines before the shot exists (the gate in DECISIONS §135) — not the rate of fire.
  {
...
// `weightClass` is the ship's MASS tier (client/src/sim-core/ship-classes.js) — a third axis, orthogonal to
// `role` (behaviour) and `class` (sound).
export const SHIPS = [
  {
  ...
  {
    // Pirate gunner (Level 4 + the side missions): a tougher skirmisher — Pirate hull (36 HP) + Pirate
    // engine (top speed +50%) + its own 50°/s thrusters, one long-range Pirate machine gun. Orange enemy_1.
    //
    // TURN RATE LOWERED 183°/s → 50°/s on 2026-08-25 (component 9 → 33), the maintainer's "make it 50 for
    // everyone except those in the intro". THIS IS THE ONLY SHIPPED ENEMY THAT CHANGED: it is the one ship
    // above 50°/s that the intro does not use. `Basic pirate ship` and `basic rocket pirate` are level-0's
    // pool and stay fast, which is what keeps the recorded archive intact (§73). Its top SPEED and
    // acceleration are untouched — thruster 33 is weight 3 exactly as Scout thrusters were, so mass is
    // still 25 and accel is still 38.0.
    ...
    components: { hull: 22, engine: 23, thruster: 33 }, stats: {
      role: 'pirate_gunner', class: 'fighter', weightClass: 'light', color: MARKER.small, reward: 50, xp: 50, dodge: 0,
  ...
  // --- Level-4 enemies (docs/plans/level-4-difficulty.md) ---
  {
    ...
    components: { hull: 24, engine: 6, thruster: 25 }, stats: {
      role: 'advanced_medium_pirate', class: 'capital', weightClass: 'medium', color: MARKER.medium, reward: 200, xp: 200, dodge: 0,
  ...
  },
  {
    ...
    components: { hull: 28, engine: 26, thruster: 27 }, stats: {
      role: 'boss2', class: 'capital', weightClass: 'heavy', color: MARKER.boss, reward: 500, xp: 500, dodge: 0,
  ...
  },
  {
    ...
    // Turn rate lowered 148°/s → 50°/s on 2026-08-25 (component 9 → 32, the same mass-31 row the lancer
    // carries). NO LIVE IMPACT: this ship is in no level's pool and no mission's — it is a catalog row kept
    // for a future harder rocketeer wave, and it is here only so the whole fighter tier is consistent when
    // it is eventually fielded. Mass stays 31 (thruster 32 is weight 3, as Scout thrusters were), so its
    // acceleration is unchanged at 30.6.
    ...
    components: { hull: 22, engine: 23, thruster: 32 }, stats: {
      role: 'advanced_rocket_pirate', class: 'fighter', weightClass: 'light', color: MARKER.small, reward: 75, xp: 75, dodge: 0,
  ...
  },
  {
    ...
    //
    // It carries component 32, 'Pirate fighter thruster' — the mass-31 row that gives 50°/s (turn rate is
    // derived from thruster power and mass, so a per-ship turn rate needs a per-mass thruster). Same weight
    // 3 as the Scout thrusters every other pirate used, so mass stays 31 and only the TURN changed — accel
    // is still 30.6. At 50°/s it turns slower than the PLAYER (115°/s) and slower than a player's ~96°/s
    // bearing sweep at the AI's standoff, which is what makes its beam escapable (DECISIONS §135).
    // It is no longer the only fighter down here: the pirate gunner and the advanced rocket pirate were
    // brought to the same 50°/s on 2026-08-25, so the three of them TIE. The intro's two ships stay fast.
    ...
    components: { hull: 22, engine: 23, thruster: 32 }, stats: {
      role: 'pirate_lancer', class: 'fighter', weightClass: 'light', color: MARKER.small, reward: 100, xp: 100, dodge: 0,
      model: { /* muzzle:auto:start */ muzzle: 1.7, exhaust: -1.7 /* muzzle:auto:end */, /* hitboxes:auto:start */ hitBoxes: [{c:{x:0.228,y:-0.132,z:0.243},h:{x:0.363,y:0.198,z:0.155},u0:{x:0.0983,y:-0.1354,z:0.9859},u1:{x:0.3103,y:0.9455,z:0.0989},u2:{x:0.9456,y:-0.2962,z:-0.135}},{c:{x:-0.007,y:0.406,z:-1.127},h:{x:0.412,y:0.333,z:0.192},u0:{x:-0.0833,y:-0.3334,z:0.9391},u1:{x:0.9937,y:0.0426,z:0.1033},u2:{x:-0.0745,y:0.9418,z:0.3278}},{c:{x:0.303,y:-0.344,z:-1.012},h:{x:0.415,y:0.316,z:0.133},u0:{x:0.9987,y:-0.0428,z:0.0269},u1:{x:-0.0182,y:0.1925,z:0.9811},u2:{x:0.0471,y:0.9804,z:-0.1915}},{c:{x:-0.02,y:-0.037,z:1.025},h:{x:0.4,y:0.216,z:0.207},u0:{x:0.0075,y:-0.133,z:0.9911},u1:{x:0.1018,y:0.9861,z:0.1316},u2:{x:0.9948,y:-0.0999,z:-0.0209}},{c:{x:0.506,y:0.011,z:-1.059},h:{x:0.361,y:0.313,z:0.145},u0:{x:-0.0299,y:-0.0474,z:0.9984},u1:{x:0.973,y:-0.23,z:0.0182},u2:{x:0.2288,y:0.972,z:0.053}},{c:{x:1.058,y:-0.208,z:-0.856},h:{x:0.505,y:0.18,z:0.119},u0:{x:0.1675,y:0.0239,z:0.9856},u1:{x:0.0893,y:0.9952,z:-0.0393},u2:{x:0.9818,y:-0.0946,z:-0.1646}},{c:{x:0.012,y:-0.19,z:1.337},h:{x:0.419,y:0.27,z:0.246},u0:{x:-0.2445,y:0.0205,z:0.9694},u1:{x:0.8558,y:-0.4654,z:0.2257},u2:{x:0.4558,y:0.8849,z:0.0963}},{c:{x:-0.439,y:-0.403,z:-1.046},h:{x:0.356,y:0.312,z:0.1},u0:{x:-0.6695,y:0.1416,z:0.7292},u1:{x:0.7164,y:-0.1363,z:0.6843},u2:{x:0.1962,y:0.9805,z:-0.0102}},{c:{x:-0.977,y:-0.146,z:-0.856},h:{x:0.502,y:0.217,z:0.169},u0:{x:-0.0546,y:-0.0112,z:0.9984},u1:{x:0.6747,y:0.7367,z:0.0452},u2:{x:0.736,y:-0.6762,z:0.0326}},{c:{x:-0.183,y:-0.146,z:-0.173},h:{x:0.284,y:0.21,z:0.139},u0:{x:-0.1687,y:0.1708,z:0.9708},u1:{x:0.1224,y:0.9809,z:-0.1513},u2:{x:0.978,y:-0.0933,z:0.1864}},{c:{x:0.102,y:0.004,z:-0.505},h:{x:0.475,y:0.377,z:0.169},u0:{x:-0.5364,y:-0.0317,z:0.8434},u1:{x:0.8419,y:-0.0898,z:0.5321},u2:{x:0.0589,y:0.9955,z:0.0749}},{c:{x:0.072,y:-0.348,z:-0.481},h:{x:0.436,y:0.28,z:0.177},u0:{x:-0.2854,y:-0.0224,z:0.9581},u1:{x:0.958,y:0.0218,z:0.2859},u2:{x:-0.
0:{x:0.0175,y:-0.0597,z:0.9981},u1:{x:0.3939,y:0.9179,z:0.048},u2:{x:0.919,y:-0.3923,z:-0.0396}},{c:{x:0.892,y:-0.306,z:-1.067},h:{x:0.297,y:0.234,z:0.1},u0:{x:0.1295,y:-0.0139,z:0.9915},u1:{x:0.9589,y:0.2565,z:-0.1217},u2:{x:-0.2526,y:0.9664,z:0.0466}},{c:{x:0.16,y:-0.22,z:0.835},h:{x:0.252,y:0.183,z:0.132},u0:{x:0.1338,y:-0.0911,z:0.9868},u1:{x:0.8896,y:-0.4278,z:-0.1601},u2:{x:0.4367,y:0.8993,z:0.0238}},{c:{x:-0.371,y:0.103,z:-1.045},h:{x:0.268,y:0.204,z:0.1},u0:{x:-0.01,y:-0.0494,z:0.9987},u1:{x:0.9972,y:0.0729,z:0.0136},u2:{x:-0.0735,y:0.9961,z:0.0485}},{c:{x:-0.359,y:-0.349,z:-1.326},h:{x:0.356,y:0.174,z:0.116},u0:{x:0.9929,y:-0.0919,z:-0.076},u1:{x:0.0319,y:-0.4099,z:0.9116},u2:{x:0.1149,y:0.9075,z:0.4041}},{c:{x:0.061,y:-0.391,z:0.938},h:{x:0.295,y:0.213,z:0.132},u0:{x:-0.1451,y:0.1724,z:0.9743},u1:{x:0.9054,y:0.4201,z:0.0605},u2:{x:-0.3989,y:0.891,z:-0.217}},{c:{x:-0.571,y:0.013,z:-1.152},h:{x:0.255,y:0.145,z:0.1},u0:{x:-0.0117,y:0.0092,z:0.9999},u1:{x:0.69,y:0.7238,z:0.0014},u2:{x:0.7237,y:-0.6899,z:0.0148}},{c:{x:0.006,y:-0.425,z:-0.047},h:{x:0.198,y:0.193,z:0.102},u0:{x:-0.3334,y:0.7006,z:-0.6308},u1:{x:0.9422,y:0.2247,z:-0.2484},u2:{x:0.0323,y:0.6772,z:0.7351}},{c:{x:-0.804,y:-0.042,z:-0.94},h:{x:0.373,y:0.209,z:0.1},u0:{x:-0.0315,y:-0.0025,z:0.9995},u1:{x:0.9896,y:0.1404,z:0.0315},u2:{x:-0.1404,y:0.9901,z:-0.002}},{c:{x:0.112,y:0.018,z:-1.378},h:{x:0.278,y:0.185,z:0.141},u0:{x:-0.1775,y:0.9239,z:0.3389},u1:{x:0.8812,y:-0.0041,z:0.4728},u2:{x:-0.4382,y:-0.3825,z:0.8134}}], broadR: 2.097 /* hitboxes:auto:end */, lift: 0.21, yaw: Math.PI, scale: 1 } /* enemy_1: lift raises the hull onto the bullet plane (assets:hitboxes coverage) */, // orange enemy_1 (faces -Z, yaw PI to face +Z)
      groups: { gun: BEAM },
...
export const LEVELS = [
  ...
  // L3; reaching it (clearing L3) shows this briefing. The hangar shop was already opened back on reaching
  // level-3 (player-facing "Level 2"); the SIDE-MISSION board unlocks HERE (progress gate, DECISIONS §91),
  // which this briefing announces — but there is no unlockShop-style action (it's derived from progress).
  // Clearly harder
...
export const MAPS = [
  {
    name: 'home-system', descriptor: {
      ...
      setpieces: [
        ...
        // kill the client lifts OOB, shows the "Finish and Return" hint, and makes this station clickable
        // (autopilot flies here → victory; since DECISIONS §143 arriving hand-flown closes it too, once the
        // button has been pressed). pos.y = -42 with client BASE_STATION_LEN 100 keeps its TOP ~y=-2.9, just
